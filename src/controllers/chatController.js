...
async function chat(ws, req, dados) {
    ...
    const chaveCriptografada = chaveService.criptografaChave(chavePublica).toString('base64');
    console.log('+ Nova conexão com o servidor estabelecida.'.green);
    ws.send(JSON.stringify({evento: 'ENVIO_CHAVE_CRIPTO', message: 'Chave criptográfica', chave: chaveCriptografada}));
